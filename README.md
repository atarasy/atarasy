...
- **Confirms with a passkey.** A member's signing key is an Ed25519 key carried inside the passkey's user handle: 32 bytes of private scalar and 32 of public key, set when the passkey is made. The household is named after that key (`key:` and the base64url SHA-256 of its public key, §13.2), and its mandate reference is that name with a label derived from the same key, so a second device rebuilds both from the handle. The public key is registered at `/_identities` under the household's name. After the person verifies, the page reads the handle, signs the canonical bytes of what is being confirmed (a decided set, §10.5; a statement; a withdrawal; a mandate version, §16.1), and discards the key; only the signature is sent. The hub's server never sees a private key and signs nothing on anyone's behalf (clause 35). Until 2026-09-16 the confirmation was the passkey's own assertion; what changed and what it costs is in `src/shared/encoding.ts`.
...
`VALENCE_EXPLORATION_RATE` has no recommended figure (specification §5) and the engine refuses to start without one; `0.2` is what `test/hub.test.ts` and the conformance suites use. Then, in this repository:

```
bun install --frozen-lockfile
VALENCE_ENGINE_URL=http://localhost:8787 VALENCE_PRESENTERS=sample-presenter bun run src/server.ts
```

Open `http://localhost:8790`, name a household and create a passkey (a platform authenticator such as Touch ID; a person has to be at the device for this step). The page then shows the household id. With both running, put a sample offer in front of that household:
