...
const {makeEngine,CONFIG_VERSION,disclosureFor}=await import(join(repo,'engine/test/helpers.ts'));
...
writeFileSync('contracts/member-review/responses.json',output);writeFileSync('ios/AtarasyCore/Tests/AtarasyCoreTests/Fixtures/member-review-responses.json',output);
