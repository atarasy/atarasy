...
const cases:any[]=[];
// §13.2, question 55. A household is the name of a key, so each fixture
// household registers its own and signs with it.
...
const digital=await call('digital-created','POST','/offers','OfferResponse',201,input('digital',HOUSE_A.household));
...
await call('other-house-created','POST','/offers','OfferResponse',201,input('digital',HOUSE_B.household));
await call('house-a-list','GET',`/offers?household=${encodeURIComponent(HOUSE_A.household)}&presenter=merchant-1`,'OfferListResponse',200);
await call('empty-presenter','GET',`/offers?household=${encodeURIComponent(HOUSE_A.household)}&presenter=fixture-other-merchant`,'OfferListResponse',200);
...
const signature=sign(null,canonicalDecisions(digital.id,decisions),HOUSE_A.privateKey).toString('base64');
...
const statementSignature=sign(null,canonicalStatement(physical.id,550,statementLines(engine.mustGet(physical.id),disputed)),HOUSE_PHYSICAL.privateKey).toString('base64');
...
const otherSignature=sign(null,canonicalStatement(physical.id,550,statementLines(engine.mustGet(physical.id),[])),HOUSE_PHYSICAL.privateKey).toString('base64');
...
const newOffer={...input('digital',HOUSE_NEW.household),config_version:config.version,candidates:[{product:'tea-a',quantity:1,is_exploration:true}]};
