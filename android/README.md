...
Recovery uses the same replicated two-of-three share layout as iOS: device `(A,B)`, recoverer `(B,C)`, host `(C,A)`, with `C = key xor A xor B`. Any distinct pair reconstructs only after its overlap matches. Recoverer packets use ephemeral P-256 ECDH, HKDF-SHA256 and AES-256-GCM bound to the exact owner, recoverer, purpose, reference and epoch. Agreement/requester keys and device shares are wrapped under the installation key. Setup requires the independently delivered `atarasyRecoveryNoticeChannel` Gradle property; request, recoverer approval and completed-key installation remain available from the Recovery screen while the private node is recovery-required.

A host move is exposed only when `atarasyMoveTargetName` and `atarasyMoveTargetOrigin` identify a different trusted environment. The app signs into the target first, re-encrypts every private record under target AAD, imports and reads back the receipt, compares offers, settlements and permissions, and obtains a target-origin attestation. Source retirement stays a separate final signed action.

Protected refresh uses an FCM data-only message containing only `profile=atarasy.member-refresh-hint.1`. Firebase options are supplied through `atarasyFirebaseAppId`, `atarasyFirebaseApiKey`, `atarasyFirebaseSenderId`, and `atarasyFirebaseProjectId`. After sign-in the app registers its Firebase Installation ID with Valence. A valid hint received in the foreground discards retained detail/review state and rereads the configured presenter sources; the messaging service never opens the bearer, private node, or offers in the background.
