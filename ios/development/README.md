...
The updated physical iPhone build displayed the mandate terms and completed native signing at 13:11 JST on 2026-09-20. The user screenshot showed `Mandate signed.`; an independent read of the dedicated development database confirmed one recorded mandate and no unsigned claim. Statement approval and interrupted-response acceptance remain separate checks. The first native attempt exposed a login-envelope/engine-assertion mismatch; it returned 404 and recorded no mandate. The corrected wire mapping is covered by exact request assertions and an encoding-removal mutation.

## Controlled lost-response acceptance

Only the Development app configuration compiles the activation path for `--acceptance-drop-statement-response`. The wrapper additionally requires environment `development` and exactly `https://api-dev.vox.delivery`. Launch with the argument only for a fresh development acceptance box, after confirming the previous box settled. No live provider is called by this fixture.

The wrapper forwards the native statement submission unchanged and waits for the real HTTP response. It consumes its one interception before suspension, discards one successful (200) statement-submit response, and records only a fixed non-secret console marker. Authentication, mandate signing, outcome reads and unsuccessful responses are not discarded. It never retries a request. This simulates response loss after the server processes the request; it does not claim a physical network outage.

Observe the unresolved result on-device and confirm the non-secret marker and server settlement independently. Terminate and relaunch without the argument, sign in, and read the saved operation's result. Verify that server logs contain exactly one submit for the operation. Do not repeat approval merely because the response was lost. A normal relaunch disables the acceptance fault; Debug, Release and UITesting do not compile its activation path.
