...
Saved operations were bound to the exact session until 2026-09-15, when they were bound to the household instead (see [member operation transport](MEMBER_OPERATION_TRANSPORT.md)). A new login session can read an earlier session's result; it still cannot sign an operation it did not prepare, because signing needs the frozen review that preparation produces. A box this flow has seen settle is remembered for the session, so a statement review loaded before settling says the box has settled instead of offering preparation. A settled box's detail reviews its settlement. There is no cross-process dispatcher, checkpoint garbage collection, power-loss durability claim or combined auth/member server in this change. The app-private store is one shared instance per composition; other processes must not write its directory.
