...
public struct MemberOfferDetail: Decodable, Equatable, Sendable {
    ...
    public static func decode(_ data: Data, expectedID: String, household: String, presenter: String? = nil) throws -> Self {
        guard let object = try? JSONSerialization.jsonObject(with: data) as? [String: Any],
              ...
              candidates.allSatisfy({ Set($0.keys) == Set("id product quantity unit_price merchant maker ships category predicted_conversion is_exploration given_by valence decided_at kept_as lineage".split(separator: " ").map(String.init)) }),
